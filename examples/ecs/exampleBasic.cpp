...
struct GameConfig {
    ...
    GameConfig() = default;
    GameConfig(const int entities, const float size) : maxEntities(entities), worldSize(size)
    {}
...
class QueryBenchmarkSystem final
    : public nexo::ecs::QuerySystem<nexo::ecs::Write<Position>, nexo::ecs::Read<Velocity>,
                                    nexo::ecs::ReadSingleton<GameConfig>, nexo::ecs::WriteSingleton<GameState> > {
   public:
    void runBenchmark()
    {
   ...
   private:
    double benchmarkQuery(int numIterations)
    {
        ...
        for (int i = 0; i < numIterations; i++) {
            ...
            for (const nexo::ecs::Entity entity : entities) {
...
class GroupBenchmarkSystem
    : public nexo::ecs::GroupSystem<nexo::ecs::Owned<nexo::ecs::Write<Position>, nexo::ecs::Read<Velocity> >,
                                    nexo::ecs::NonOwned<>, nexo::ecs::WriteSingleton<GameConfig>,
   ...
   private:
    ...
    [[nodiscard]] double benchmarkEach(int numIterations) const
    {
        // Method 2: Using the "each" method, faster than the iterator, but does not necessary enforce constness if not
        // mentioned
        const auto start = std::chrono::high_resolution_clock::now();
        ...
        for (int i = 0; i < numIterations; i++) {
            m_group->each([]([[maybe_unused]] nexo::ecs::Entity entity, Position& position, const Velocity& velocity) {
                ...
                // This triggers a compilation error since we are using a const reference to velocity in the lambda
                // function velocity.x += 1;
            });
    ...
    double benchmarkSpans(int numIterations)
    {
        ...
        for (int i = 0; i < numIterations; i++) {
            ...
            // Constness is not enforced on the span itself since it is basically a non-owning view of the underlying
            // data. But we consider it to be good practice to make more explicit that we are using read-only components
            const auto velocitySpan = get<Velocity>();
            // We can safely update the game config
            gameConfig.maxEntities += 1000;

            const size_t size = positionSpan.size();
            for (size_t j = 0; j < size; ++j) {
                ...
                // This triggers a compilation error since velocity is read-only even though we did not explicitly mark
                // it as const velocity.x += 1;
...
int main()
{
    ...
    for (int i = 0; i < 5000; ++i) {
        ...
        const auto velX = static_cast<float>(velocityDist(gen));
        const auto velY = static_cast<float>(velocityDist(gen));
    ...
    const auto& gameState = coordinator.getSingletonComponent<GameState>();
    log("Game time: " + std::to_string(gameState.gameTime));

    const auto& gameConfig = coordinator.getSingletonComponent<GameConfig>();
    log("Max entities: " + std::to_string(gameConfig.maxEntities));
