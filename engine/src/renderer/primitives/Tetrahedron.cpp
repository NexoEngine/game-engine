...
namespace nexo::renderer {
     ...
     */
    static void genTetrahedronMesh(std::array<glm::vec3, 12>& vertices, std::array<glm::vec2, 12>& texCoords,
                                   std::array<glm::vec3, 12>& normals)
    {
        ...
        std::array<glm::vec3, 12> verts = {v0, v1, v2, v0, v2, v3, v0, v3, v1, v1, v3, v2};
        ...
        for (int i = 0; i < 12; i += 3) {
            const glm::vec3 normal =
                glm::normalize(glm::cross(vertices[i + 1] - vertices[i], vertices[i + 2] - vertices[i]));
    ...
    std::shared_ptr<NxVertexArray> NxRenderer3D::getTetrahedronVAO()
    {
        ...
        if (tetrahedronVao) return tetrahedronVao;
        ...
        for (uint32_t i = 0; i < nbVerticesTetrahedron; ++i) indices[i] = i;
