...
namespace nexo::renderer {
     ...
     */
    static void genPyramidMesh(std::array<glm::vec3, 18>& vertices, std::array<glm::vec2, 18>& texCoords,
                               std::array<glm::vec3, 18>& normals)
    {
        ...
        constexpr std::array<glm::vec3, 18> verts = {
            v1, v2, v3, v1, v3, v4,                        // Base face
            v0, v2, v1, v0, v3, v2, v0, v4, v3, v0, v1, v4 // Side faces
        };
        std::ranges::copy(verts, vertices.begin());

        // Basic UV mapping for each face
        constexpr std::array<glm::vec2, 18> texturesCoord = {
            glm::vec2(0.5f, 0.0f), glm::vec2(0.0f, 1.0f), glm::vec2(1.0f, 1.0f), glm::vec2(0.5f, 0.0f),
            glm::vec2(1.0f, 1.0f), glm::vec2(0.0f, 1.0f),
            // Side faces
            glm::vec2(0.5f, 1.0f), glm::vec2(0.0f, 0.0f), glm::vec2(1.0f, 0.0f), glm::vec2(0.5f, 1.0f),
            glm::vec2(0.0f, 0.0f), glm::vec2(1.0f, 0.0f), glm::vec2(0.5f, 1.0f), glm::vec2(0.0f, 0.0f),
            glm::vec2(1.0f, 0.0f), glm::vec2(0.5f, 1.0f), glm::vec2(0.0f, 0.0f), glm::vec2(1.0f, 0.0f)};
        std::ranges::copy(texturesCoord, texCoords.begin());

        // Compute normals for each face of the pyramid
        std::array<glm::vec3, 18> norm{};
        for (int i = 0; i < 18; i += 3) {
            const glm::vec3 normal = glm::normalize(glm::cross(verts[i + 1] - verts[i], verts[i + 2] - verts[i]));
    ...
    std::shared_ptr<NxVertexArray> NxRenderer3D::getPyramidVAO()
    {
        ...
        if (pyramidVao) return pyramidVao;
        ...
        for (uint32_t i = 0; i < nbVerticesPyramid; ++i) indices[i] = i;
