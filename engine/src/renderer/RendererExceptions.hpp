...
namespace nexo::renderer {
    ...
    class NxGraphicsApiNotInitialized final : public Exception {
       public:
        explicit NxGraphicsApiNotInitialized(const std::string &backendApiName,
                                             const std::source_location loc = std::source_location::current())
            : Exception(std::format("[{}] API is not initialized, call the init function first", backendApiName), loc)
        {}
    };
    ...
    class NxGraphicsApiViewportResizingFailure final : public Exception {
       public:
        explicit NxGraphicsApiViewportResizingFailure(const std::string &backendApi, const bool tooBig,
                                                      ...
                                                      const std::source_location loc = std::source_location::current())
            : Exception(std::format("[{}] Viewport resizing failed: {}x{} is too {}", backendApi, width, height,
                                    (tooBig ? "big" : "small")),
                        loc)
        {}
    };
    ...
    class NxFramebufferResizingFailed final : public Exception {
       public:
        explicit NxFramebufferResizingFailed(const std::string &backendApi, const bool tooBig, const unsigned int width,
                                             const unsigned int height,
                                             const std::source_location loc = std::source_location::current())
            : Exception(std::format("[{}] Framebuffer resizing failed: {}x{} is too {}", backendApi, width, height,
                                    (tooBig ? "big" : "small")),
                        loc)
        {}
    };
    ...
    class NxFramebufferReadFailure final : public Exception {
       public:
        explicit NxFramebufferReadFailure(const std::string &backendApiName, int index, int x, int y,
                                          const std::source_location loc = std::source_location::current())
            : Exception(std::format("[{}] Unable to read framebuffer with index {} at coordinate ({}, {})",
                                    backendApiName, index, x, y),
                        loc)
        {}
    };
    ...
    class NxFramebufferInvalidIndex final : public Exception {
       public:
        explicit NxFramebufferInvalidIndex(const std::string &backendApiName, int index,
                                           const std::source_location loc = std::source_location::current())
            : Exception(std::format("[{}] Invalid attachment index : {}", backendApiName, index), loc){};
    };
       ...
       public:
        explicit NxRendererNotInitialized(const NxRendererType type,
            ...
            : Exception(std::format("{} Renderer not initialized, call the init function first",
                                    (type == NxRendererType::RENDERER_2D ? "[RENDERER 2D]" : "[RENDERER 3D]")),
                        loc)
        {}
    };
    ...
    class NxRendererSceneLifeCycleFailure final : public Exception {
       public:
        explicit NxRendererSceneLifeCycleFailure(const NxRendererType type, const std::string &msg,
                                                 const std::source_location loc = std::source_location::current())
            : Exception(
                  std::format("{} {}", (type == NxRendererType::RENDERER_2D ? "[RENDERER 2D]" : "[RENDERER 3D]"), msg),
                  loc)
        {}
    };
    ...
    class NxTextureInvalidSize final : public Exception {
       public:
        explicit NxTextureInvalidSize(const std::string &backendApi, const unsigned int width,
                                      const unsigned int height, const unsigned int maxTextureSize,
                                      const std::source_location loc = std::source_location::current())
            : Exception(std::format("[{}] Invalid size for texture: {}x{} is too big, max texture size is : {}",
                                    backendApi, width, height, maxTextureSize),
                        loc)
        {}
    };
    ...
    class NxTextureUnsupportedFormat final : public Exception {
       public:
        explicit NxTextureUnsupportedFormat(const std::string &backendApi, const int channels, const std::string &path,
                                            const std::source_location loc = std::source_location::current())
            : Exception(std::format("[{}] Unsupported image format with {} channels in {}", backendApi, channels, path),
                        loc)
        {}
    };
    ...
    class NxTextureSizeMismatch final : public Exception {
       public:
        explicit NxTextureSizeMismatch(const std::string &backendApi, const size_t dataSize, const size_t expectedSize,
                                       const std::source_location loc = std::source_location::current())
            : Exception(std::format("[{}] Data size does not match the texture size: {} != {}", backendApi, dataSize,
                                    expectedSize),
                        loc)
        {}
    };
    ...
    class NxPipelineRenderTargetNotSetException final : public Exception {
       public:
        explicit NxPipelineRenderTargetNotSetException(const std::source_location loc = std::source_location::current())
