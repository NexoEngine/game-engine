...
namespace nexo::renderer {
    ...
    {
        if (const size_t expectedSize = static_cast<size_t>(m_width) * m_height * (m_dataFormat == GL_RGBA ? 4 : 3);
            size != expectedSize)
            THROW_EXCEPTION(NxTextureSizeMismatch, "OPENGL", size, expectedSize);
        glBindTexture(GL_TEXTURE_2D, m_id);
        // Update the entire texture with new data
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, static_cast<int>(m_width), static_cast<int>(m_height), m_dataFormat,
                        GL_UNSIGNED_BYTE, data);
        glBindTexture(GL_TEXTURE_2D, 0);
    }

    void NxOpenGlTexture2D::ingestDataFromStb(const uint8_t *data, const int width, const int height,
                                              const int channels, const std::string &debugPath)
    {
        GLint internalFormat = 0;
        GLenum dataFormat    = 0;
        switch (channels) {
        ...
        createOpenGLTexture(data, static_cast<unsigned int>(width), static_cast<unsigned int>(height), internalFormat,
                            dataFormat);
    }
