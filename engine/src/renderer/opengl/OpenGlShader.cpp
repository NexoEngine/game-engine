...
namespace nexo::renderer {
    ...
    /**
     * @brief Converts a shader type string to the corresponding OpenGL constant.
     *
     * This function takes a string representing the shader type (`vertex` or `fragment`)
     * and returns the corresponding OpenGL constant (`GL_VERTEX_SHADER` or `GL_FRAGMENT_SHADER`).
     * If the type is not recognized, the function returns 0.
     *
     * @param type String representing the shader type.
     * @return The GLenum constant corresponding to the shader type, or 0 if unknown.
     */
    ...
    void NxOpenGlShader::compile(const std::unordered_map<GLenum, std::string> &shaderSources)
    {
        ...
        if (isLinked == GL_FALSE) {
            ...
            for (const auto id : glShaderIds) glDeleteShader(id);
        ...
        for (const auto id : glShaderIds) glDetachShader(program, id);
    }
    ...
    bool NxOpenGlShader::setUniformFloat(const std::string &name, const float value) const
    {
        ...
        if (NxShader::setUniformFloat(name, value)) return true; // Value was cached, no need to update
    ...
    bool NxOpenGlShader::setUniformFloat(const NxShaderUniforms uniform, const float value) const
    {
        ...
        if (!NxShader::hasUniform(name)) return false;
        if (NxShader::setUniformFloat(name, value)) return true;
    ...
    {
        if (!NxShader::hasUniform(name)) return false;
        if (NxShader::setUniformFloat2(name, values)) return true;
    ...
    {
        if (!NxShader::hasUniform(name)) return false;
        if (NxShader::setUniformFloat3(name, values)) return true;
    ...
    {
        if (!NxShader::hasUniform(name)) return false;
        if (NxShader::setUniformFloat4(name, values)) return true;
    ...
    {
        if (!NxShader::hasUniform(name)) return false;
        if (NxShader::setUniformMatrix(name, matrix)) return true;
    ...
    {
        if (!NxShader::hasUniform(name)) return false;
        if (NxShader::setUniformInt(name, value)) return true;
    ...
    {
        if (!NxShader::hasUniform(name)) return false;
        if (NxShader::setUniformBool(name, value)) return true;
    ...
    bool NxOpenGlShader::setUniformIntArray(const NxShaderUniforms uniform, const int *values,
