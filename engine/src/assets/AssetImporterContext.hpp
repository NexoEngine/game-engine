...
namespace nexo::assets {
    ...
    struct AssetImporterContext {
       public:
         ...
         *
         * This method creates a candidate asset location by using the current location as a base, then sets a unique
         * name by incrementing an internal dependency counter and formatting the name with a dedicated formatting
         * function. If the generated location already exists in the asset catalog, the method continues to update the
         * candidate location until a unique one is found or the maximum allowed dependency count is exceeded. In the
         * latter case, an error is logged, and the last candidate is returned.
         *
       ...
       private:
        std::unique_ptr<IAsset> m_mainAsset =
            nullptr; //< Main asset being imported, resulting asset (MUST be set by importer)
    ...
    AssetLocation AssetImporterContext::genUniqueDependencyLocation()
    {
        ...
        if (!AssetCatalog::getInstance().getAsset(depLoc)) return depLoc;
        ...
        while (AssetCatalog::getInstance().getAsset(depLoc)) {
            ...
            if (m_depUniqueId > ASSET_MAX_DEPENDENCIES) {
                ...
                LOG(NEXO_ERROR, "Failed to generate unique name for asset: {}: couldn't find unique id",
                    depLoc.getFullLocation());
                break;
            }
        }
        return depLoc;
