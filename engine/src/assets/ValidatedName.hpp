...
namespace nexo::assets {
    ...
    class InvalidName final : public Exception {
       public:
        explicit InvalidName(std::string_view name, std::string_view message,
    ...
    concept Validator = requires(std::string_view name) {
        {
            T::validate(name)
        } -> std::same_as<std::optional<std::string>>; //< The validation method.
    };
    ...
    {
        if (auto errorMessage = validate(name); errorMessage.has_value()) return errorMessage;
        _value = name;
        return std::nullopt;
    }
} // namespace nexo::assets
