...
namespace nexo::assets {
    ...
    class AssetImporter {
       public:
         ...
         *
         * This function attempts to import an asset by invoking the given importer. It utilizes a custom import context
         * if one is configured (see setCustomContext()); otherwise, it creates a temporary context initialized with the
         * provided input data and location. After importing, if the asset is valid, the function ensures that the asset
         * has a unique identifier and updates its location metadata if it is set to "default". The asset is then
         * registered in the AssetCatalog. If the import fails, a null asset reference is returned.
         *
         ...
         * @return GenericAssetRef A reference to the imported and registered asset, or a null reference if the import
         * fails.
         */
        GenericAssetRef importAssetUsingImporter(const AssetLocation& location,
                                                 const ImporterInputVariant& inputVariant,
                                                 AssetImporterBase* importer) const;
         ...
         * @return GenericAssetRef A reference to the successfully imported asset, or a null reference if the import
         * fails.
         */
        [[nodiscard]] GenericAssetRef importAssetTryImporters(const AssetLocation& location,
                                                              const ImporterInputVariant& inputVariant,
         ...
         */
        [[nodiscard]] const ImporterMap& getImporters() const
        {
            return m_importers;
        }
        ...
        void setCustomContext(AssetImporterContext* ctx)
        {
            m_customCtx = ctx;
        }
         ...
         */
        [[nodiscard]] AssetImporterContext* getCustomContext() const
        {
            return m_customCtx;
        }
       ...
       protected:
         ...
         * @param priority The registration priority. Importers with equal priorities retain the order in which they
         * were added.
         */
        template<typename AssetType, typename ImporterType>
            requires std::derived_from<AssetType, IAsset> && std::derived_from<ImporterType, AssetImporterBase>
         ...
         * @param priority An integer representing the importer's priority; higher values denote higher precedence. If
