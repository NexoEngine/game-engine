...
namespace nexo::components {
    [[nodiscard]] glm::mat4 CameraComponent::getProjectionMatrix() const
    {
        if (type == CameraType::PERSPECTIVE) {
            return glm::perspective(glm::radians(fov), static_cast<float>(width) / static_cast<float>(height),
                                    nearPlane, farPlane);
        }
        return glm::ortho(0.0f, static_cast<float>(width), static_cast<float>(height), 0.0f, nearPlane, farPlane);
    }

    [[nodiscard]] glm::mat4 CameraComponent::getViewMatrix(const TransformComponent& transf)
    {
    ...
    {
        return {width, height, viewportLocked, fov, nearPlane, farPlane, type, clearColor, main, m_renderTarget};
    }
} // namespace nexo::components
