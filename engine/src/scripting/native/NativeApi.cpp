...
namespace nexo::scripting {
    ...
    ecs::Entity NxCreatePointLight(const Vector3 position, const Vector4 color, const float linear,
    ...
    ecs::Entity NxCreateTetrahedron(const Vector3 position, const Vector3 size, const Vector3 rotation,
                                    const Vector4 color)
    {
        auto& app                = Application::getInstance();
    ...
    ecs::Entity NxCreateCylinder(const Vector3 position, const Vector3 size, const Vector3 rotation,
                                 const Vector4 color, const UInt32 nbSegment)
    {
        auto& app                = Application::getInstance();
    ...
    ecs::Entity NxCreateSphere(const Vector3 position, const Vector3 size, const Vector3 rotation, const Vector4 color,
                               const UInt32 nbSubdivision)
    {
        auto& app                = Application::getInstance();
    ...
    Int64 NxRegisterComponent(const char* name, const UInt64 componentSize, const Field* fields,
                              const UInt64 fieldCount)
    {
        ...
        for (UInt64 i = 0; i < fieldCount; ++i) {
            ...
            LOG(NEXO_DEV, "Registering field {}: {} of type {}", i, static_cast<char*>(fields[i].name),
                static_cast<UInt64>(fields[i].type));
        }
        ...
        static_assert(static_cast<uint64_t>(ecs::FieldType::_Count) == static_cast<uint64_t>(FieldType::_Count),
                      "FieldType enum value count mismatch");
        for (UInt64 i = 0; i < fieldCount; ++i) {
        ...
        coordinator.addComponentDescription(componentType, ecs::ComponentDescription{
                                                               .name   = name,
    ...
    void NxCreateBodyFromShape(ecs::Entity entity, Vector3 position, Vector3 size, Vector3 rotation, UInt32 shapeType,
                               UInt32 motionType)
    {
