...
namespace nexo::system {
	...
	static void setupLights(std::shared_ptr<renderer::Shader> shader, const components::LightComponent& lightContext)
	{
        shader->bind();
        ...
        for (unsigned int i = 0; i < lightContext.spotLightCount; ++i)
        {
            ...
            shader->setUniformFloat4(std::format("spotLights[{}].color", i), spotLight.color);
