...
namespace nexo::system {
    ...
    void PerspectiveCameraControllerSystem::update(const Timestep ts)
    {
        ...
        for (const ecs::Entity entity : entities) {
            ...
            if (!sceneTag.isActive || sceneTag.id != sceneRendered) continue;
            ...
            if (event::isKeyPressed(NEXO_KEY_SHIFT)) cameraController.translationSpeed = 10.0f;
            if (event::isKeyReleased(NEXO_KEY_SHIFT)) cameraController.translationSpeed = 5.0f;
    ...
    void PerspectiveCameraControllerSystem::handleEvent(event::EventMouseScroll &event)
    {
        ...
        for (const ecs::Entity entity : entities) {
            ...
            if (!sceneTag.isActive || sceneTag.id != sceneRendered || !cameraComponent.active) continue;
            auto &transform = getComponent<components::TransformComponent>(entity);
            glm::vec3 front = transform.quat * glm::vec3(0.0f, 0.0f, -1.0f);
    ...
    void PerspectiveCameraControllerSystem::handleEvent(event::EventMouseMove &event)
    {
        ...
        for (const ecs::Entity entity : entities) {
            ...
            const glm::vec2 mouseDelta =
                (currentMousePosition - controller.lastMousePosition) * controller.mouseSensitivity;
            ...
            glm::quat yawRotation =
                glm::angleAxis(glm::radians(-mouseDelta.x), glm::vec3(0.0f, 1.0f, 0.0f)); // World up for yaw
    ...
    void PerspectiveCameraTargetSystem::handleEvent(event::EventMouseScroll &event)
    {
        ...
        for (const ecs::Entity entity : entities) {
            ...
            const auto &cameraComponent = getComponent<components::CameraComponent>(entity);
            if (!tag.isActive || sceneRendered != tag.id || !cameraComponent.active) continue;
            auto &target = getComponent<components::PerspectiveCameraTarget>(entity);
            target.distance -= event.y * zoomSpeed;
            if (target.distance < 0.1f) target.distance = 0.1f;
            ...
            if (glm::length(offset) < 0.001f) offset = glm::vec3(0, 0, 1);
    ...
    void PerspectiveCameraTargetSystem::handleEvent(event::EventMouseMove &event)
    {
        ...
        for (const ecs::Entity entity : entities) {
            ...
            if (!sceneTag.isActive || sceneTag.id != sceneRendered || cameraComponent.resizing ||
                !event::isMouseDown(NEXO_MOUSE_RIGHT) || !cameraComponent.active) {
            ...
            const auto &transformTargetComponent =
                getComponent<components::TransformComponent>(targetComponent.targetEntity);
            ...
            if (glm::dot(front, glm::vec3(0, 1, 0)) * sgn(yAngle) > 0.99f) yAngle = 0.0f;
