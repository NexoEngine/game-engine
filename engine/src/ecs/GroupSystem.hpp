...
namespace nexo::ecs {
    ...
    class GroupSystem
        : public AGroupSystem,
          public SingletonComponentMixin<GroupSystem<OwnedAccess, NonOwnedAccess, SingletonAccessTypes...>,
       ...
       private:
        ...
        /**
         * @brief Helper to convert a tuple type to a type list (parameter pack)
         *
         * This utility unpacks the types from a std::tuple and returns them as a new std::tuple.
         * It is used to facilitate the creation of type lists from tuple types.
         *
         * @tparam Tuple The input tuple type containing component types
         * @tparam I Index sequence to unpack the tuple
         * @return A new std::tuple containing the unpacked types
         */
        ...
        /**
         * @brief Overload to create a type list from a tuple type
         *
         * This function generates an index sequence based on the size of the input tuple
         * and calls the implementation function to unpack the types.
         *
         * @tparam Tuple The input tuple type containing component types
         * @return A new std::tuple containing the unpacked types
         */
        ...
        struct GetComponentAccess {
            ...
            static constexpr AccessType accessType = OwnedTrait::found    ? OwnedTrait::accessType :
        ...
        class ComponentSpan {
           ...
           public:
            ...
            auto operator[](size_t index)
                -> std::conditional_t<GetComponentAccess<std::remove_const_t<U>>::accessType == AccessType::Write,
                                      std::remove_const_t<U>&, const std::remove_const_t<U>&>
            {
       ...
       public:
        ...
        friend class SingletonComponentMixin<GroupSystem<OwnedAccess, NonOwnedAccess, SingletonAccessTypes...>,
                                             SingletonAccessTypes...>;
        ...
        GroupSystem()
        {
            static_assert(std::tuple_size_v<OwnedTypes> > 0, "GroupSystem must have at least one owned component type");

            if (!coord) THROW_EXCEPTION(InternalError, "Coordinator is null in GroupSystem constructor");

            m_group = createGroupImpl(tupleToTypeList<OwnedTypes>(), tupleToTypeList<NonOwnedTypes>());
        ...
        auto get()
        {
            if (!m_group) THROW_EXCEPTION(InternalError, "Group is null in GroupSystem");
            ...
            if constexpr (isOwned) {
                ...
                return ComponentSpan<
                    std::conditional_t<GetComponentAccess<T>::accessType == AccessType::Read, const T, T>>(baseSpan);
            } else {
        ...
        std::shared_ptr<ActualGroupType> createGroupImpl(std::tuple<OT...>, std::tuple<NOT...>)
        {
            if constexpr (sizeof...(OT) > 0) {
                if constexpr (sizeof...(NOT) > 0) {
                    ...
                    if (!group) THROW_EXCEPTION(InternalError, "Group is null in GroupSystem");
