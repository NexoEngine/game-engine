...
namespace nexo::ecs {
    ...
    class SingletonComponent final : public ISingletonComponent {
       public:
        ...
        explicit SingletonComponent(Args&&... args) : _instance(std::forward<Args>(args)...)
        {}
       ...
       public:
        ...
        void registerSingletonComponent(Args&&... args)
        {
            ...
            if (m_singletonComponents.contains(typeName)) {
                LOG(NEXO_WARN,
                    "ECS::SingletonComponentManager::registerSingletonComponent: trying to register a singleton "
                    "component more than once");
                return;
            }
            m_singletonComponents.insert(
                {typeName, std::make_shared<SingletonComponent<T>>(std::forward<Args>(args)...)});
        }
        ...
        T& getSingletonComponent()
        {
            ...
            if (!m_singletonComponents.contains(typeName)) THROW_EXCEPTION(SingletonComponentNotRegistered);
