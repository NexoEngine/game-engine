...
namespace nexo::ecs {
     ...
     *
     * Example: if Tuple is std::tuple<std::shared_ptr<ComponentArray<Position>>,
     * std::shared_ptr<ComponentArray<Velocity>>> then tuple_contains_component<T, Tuple>::value is true if T is
    ...
    class Group final : public IGroup {
       public:
        ...
        Group(OwnedTuple ownedArrays, NonOwnedTuple nonOwnedArrays)
            : m_ownedArrays(std::move(ownedArrays)), m_nonOwnedArrays(std::move(nonOwnedArrays))
        {
            m_ownedSignature = std::apply(
                []([[maybe_unused]] auto&&... arrays) {
                    Signature signature;
            ...
            const Signature nonOwnedSignature = std::apply(
                []([[maybe_unused]] auto&&... arrays) {
                    Signature signature;
                    ((signature.set(getComponentTypeID<typename std::decay_t<decltype(*arrays)>::component_type>())),
                     ...);
         ...
         */
        [[nodiscard]] bool sortingInvalidated() const
        {
            return m_sortingInvalidated;
        }
         ...
         */
        [[nodiscard]] const Signature& ownedSignature() const
        {
            return m_ownedSignature;
        }
         ...
         * @return const Signature& Combined signature.
         */
        [[nodiscard]] const Signature& allSignature() const override
        {
            return m_allSignature;
        }
        ...
        class GroupIterator {
           public:
             ...
             */
            GroupIterator(const Group* view, std::size_t index) : m_view(view), m_index(index)
            {}
            ...
            {
                if (m_index >= m_view->size()) THROW_EXCEPTION(OutOfRange, m_index);
        ...
        {
            return GroupIterator(this, size());
        }
        ...
        {
            auto firstArray = std::get<0>(m_ownedArrays);
            if (!firstArray) THROW_EXCEPTION(InternalError, "Component array is null");

            for (std::size_t i = 0; i < firstArray->groupSize(); ++i) {
                Entity e = firstArray->getEntityAtIndex(i);
                callFunc(func, e, std::make_index_sequence<std::tuple_size_v<OwnedTuple>>{},
        ...
        void eachInRange(size_t startIndex, const size_t count, Func func) const
        {
            ...
            if (startIndex >= firstArray->groupSize()) return; // Nothing to iterate
        ...
        {
            std::apply([e](auto&&... arrays) { ((arrays->addToGroup(e)), ...); }, m_ownedArrays);
        ...
        {
            std::apply([e](auto&&... arrays) { ((arrays->removeFromGroup(e)), ...); }, m_ownedArrays);
        ...
        void sortBy(FieldExtractor<CompType, FieldType> extractor, bool ascending = true)
        {
            ...
            if (!m_sortingInvalidated) return;
            ...
            for (size_t i = 0; i < groupSize; i++) entities.push_back(drivingArray->getEntityAtIndex(i));
            ...
            std::ranges::sort(entities, [&](Entity a, Entity b) {
                ...
                if (ascending) return extractor(compA) < extractor(compB);
        ...
        {
            for (auto& [_, storage] : m_partitionStorageMap) storage->markDirty();
        }
       ...
       private:
        ...
        {
            std::apply([&](auto&&... arrays) { ((reorderArray(arrays, newOrder)), ...); }, m_ownedArrays);
        }
        ...
        void reorderArray(ArrayPtr array, const std::vector<Entity>& newOrder) const
        {
            ...
            if (newOrder.size() != groupSize) THROW_EXCEPTION(InternalError, "New order size doesn't match group size");
        ...
        auto dereference(std::size_t index) const
        {
            ...
            auto ownedData = std::apply(
                [entity](auto&&... arrays) { return std::forward_as_tuple(arrays->get(entity)...); }, m_ownedArrays);
         ...
         * @param e Entity.
         * @param ownedIndices index_sequence for owned components.
         * @param nonOwnedIndices index_sequence for non‑owned components.
         */
        ...
        std::unordered_map<std::string, std::unique_ptr<IPartitionStorage>>
