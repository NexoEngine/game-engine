...
#define LISTENS_TO(...) \
   public               \
    event::Listens<__VA_ARGS__>

namespace nexo::event {

    template<typename L, typename E>
    concept HandlesEvent = std::derived_from<L, BaseListener> && requires(L* l, E& e) {
        {
            l->handleEvent(e)
        } -> std::same_as<void>;
    };
     ...
     *
     * Implements the trigger() method to dispatch the event to a listener that can handle the specific DerivedEvent
     * type.
     *
    ...
    class Event : public IEvent {
       ...
       protected:
        ...
        {
            if (auto* p = dynamic_cast<Listener<T>*>(&listener)) return p->handleEvent(event);
        ...
        void unregisterListener(BaseListener* listener)
        {
            ...
            if (const auto it = m_listeners.find(typeIndex); it != m_listeners.end()) {
                ...
                if (const auto listenerIt = std::ranges::find(listeners, listener); listenerIt != listeners.end()) {
                    ...
                    LOG(NEXO_DEV, "EventManager(unregisterListener): Unregistered listener {}",
