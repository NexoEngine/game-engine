...
namespace nexo::core {
    ...
    class LoadModelException final : public Exception {
       public:
        explicit LoadModelException(const std::string &filePath, const std::string &errorStr,
                                    const std::source_location loc = std::source_location::current())
            : Exception("Failure to load model : " + filePath + " : " + errorStr, loc){};
    };

    class SceneManagerLifecycleException final : public Exception {
       public:
    ...
    class TooManyPointLightsException final : public Exception {
       public:
        explicit TooManyPointLightsException(unsigned int sceneRendered, size_t nbPointLights,
                                             const std::source_location loc = std::source_location::current())
            : Exception(std::format("Too many point lights ({} > {}) in scene [{}]", nbPointLights, MAX_POINT_LIGHTS,
                                    sceneRendered),
                        loc)
        {}
    };

    class TooManySpotLightsException final : public Exception {
       public:
        explicit TooManySpotLightsException(unsigned int sceneRendered, size_t nbSpotLights,
                                            const std::source_location loc = std::source_location::current())
            : Exception(std::format("Too many spot lights ({} > {}) in scene [{}]", nbSpotLights, MAX_SPOT_LIGHTS,
                                    sceneRendered),
                        loc)
        {}
    };
