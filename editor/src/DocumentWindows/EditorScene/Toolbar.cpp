...
//  Author:      Mehdy MORVAN
//  Date:        28/04/2025
//  Description: Source file for the toolbar rendering of the editor scene
//
///////////////////////////////////////////////////////////////////////////////
...
namespace nexo::editor {
                                          ...
                                          const std::vector<ImNexo::GradientStop>& gradientStop, bool* rightClicked)
    {
        ...
        if (rightClicked != nullptr) *rightClicked = ImGui::IsItemClicked(ImGuiMouseButton_Right);
        return clicked;
    }
    ...
    void EditorScene::renderSnapSubMenu(const ImVec2& snapButtonPos, const ImVec2& buttonSize, bool& showSnapMenu)
    {
        const std::vector<ImNexo::ButtonProps> buttonProps = {
            {.uniqueId       = "toggle_translate_snap",
             .icon           = ICON_FA_TH,
             .onClick        = [this]() { this->m_snapTranslateOn = !this->m_snapTranslateOn; },
             .onRightClick   = [this]() { this->m_popupManager.openPopup("Snap settings popup", ImVec2(400, 140)); },
            ...
            {.uniqueId       = "toggle_rotate_snap",
             ...
             .onClick        = [this]() { this->m_snapRotateOn = !m_snapRotateOn; },
             .onRightClick   = [this]() { this->m_popupManager.openPopup("Snap settings popup", ImVec2(400, 140)); },
    ...
    void EditorScene::snapSettingsPopup()
    {
        if (m_popupManager.showPopupModal("Snap settings popup")) {
            ...
            if (ImGui::BeginTable("TranslateSnap", 4, ImGuiTableFlags_SizingStretchProp)) {
            ...
            if (ImGui::BeginTable("ScaleAndRotateSnap", 4, ImGuiTableFlags_SizingStretchProp)) {
                ImGui::TableSetupColumn("##Label",
                                        ImGuiTableColumnFlags_WidthStretch | ImGuiTableColumnFlags_NoHeaderLabel);
                ImGui::TableSetupColumn("##Value",
                                        ImGuiTableColumnFlags_WidthStretch | ImGuiTableColumnFlags_NoHeaderLabel);
                // Empty columns to match the first table's structure
                ImGui::TableSetupColumn("##Empty1",
                                        ImGuiTableColumnFlags_WidthStretch | ImGuiTableColumnFlags_NoHeaderLabel);
                ImGui::TableSetupColumn("##Empty2",
                                        ImGuiTableColumnFlags_WidthStretch | ImGuiTableColumnFlags_NoHeaderLabel);
    ...
    void EditorScene::gridSettingsPopup()
    {
        if (m_popupManager.showPopupModal("Grid settings")) {
            ...
            if (ImGui::BeginTable("GridSettings", 2, ImGuiTableFlags_SizingStretchProp)) {
                ImGui::TableSetupColumn("##Label",
                                        ImGuiTableColumnFlags_WidthStretch | ImGuiTableColumnFlags_NoHeaderLabel);
                ImGui::TableSetupColumn("##X",
                                        ImGuiTableColumnFlags_WidthStretch | ImGuiTableColumnFlags_NoHeaderLabel);
                ImNexo::RowDragFloat1("Grid size", "", &gridSettings.gridSize, 50.0f, 150.0f);
                if (ImGui::IsItemHovered()) ImGui::SetTooltip("The total size of the grid");
                ImNexo::RowDragFloat1("Pixel cell spacing", "", &gridSettings.minPixelsBetweenCells, 0.0f, 100.0f,
                                      0.1f);
                if (ImGui::IsItemHovered()) ImGui::SetTooltip("Level of detail of internal cells");
                ImNexo::RowDragFloat1("Cell size", "", &gridSettings.cellSize, 0.1f, 20.0f, 0.02f);
                if (ImGui::IsItemHovered()) ImGui::SetTooltip("The size of the internal cells");
                ImGui::EndTable();
            }
        ...
        if (m_activeCamera == m_editorCamera) {
            if (renderToolbarButton("editor_camera", ICON_FA_CAMERA, "Edit Editor Camera Setting", m_buttonGradient)) {
                const auto& uuidComponent =
                    Application::m_coordinator->getComponent<components::UuidComponent>(m_editorCamera);
                selector.addToSelection(uuidComponent.uuid, m_editorCamera);
            }
        } else {
            if (renderToolbarButton("switch_back", ICON_FA_EXCHANGE, "Switch back to editor camera",
                                    m_buttonGradient)) {
                                                   ...
                                                   ImNexo::ButtonProps& inactiveGizmoMode)
    {
        ...
        return renderToolbarButton(activeGizmoMode.uniqueId, activeGizmoMode.icon, activeGizmoMode.tooltip,
                                   showGizmoModeMenu ? m_selectedGradient : m_buttonGradient);
    }
    ...
    void EditorScene::renderToolbar()
    {
        ...
        bool addPrimitiveClicked      = renderToolbarButton("add_primitive", ICON_FA_PLUS_SQUARE, "Add primitive",
                                                       showPrimitiveMenu ? m_selectedGradient : m_buttonGradient);
        if (addPrimitiveClicked) showPrimitiveMenu = !showPrimitiveMenu;
        ...
        std::vector<ImNexo::ButtonProps> gizmoButtons = {gizmoTranslateButtonProps, gizmoRotateButtonProps,
                                                         gizmoScaleButtonProps, gizmoUniversalButtonProps};

        ImNexo::ButtonProps activeOp;
        auto eraseGizmoButton = [&activeOp, &gizmoButtons](const std::string& id, const ImNexo::ButtonProps& prop) {
            activeOp = prop;
            std::erase_if(gizmoButtons, [&id](const auto& p) { return p.uniqueId == id; });
        };
        ...
        bool changeGizmoOpClicked   = renderToolbarButton(activeOp.uniqueId, activeOp.icon, activeOp.tooltip,
                                                        showGizmoOpMenu ? m_selectedGradient : m_buttonGradient);
        if (changeGizmoOpClicked) showGizmoOpMenu = !showGizmoOpMenu;
        ...
        bool changeGizmoModeClicked =
            renderGizmoModeToolbarButton(showGizmoModeMenu, activeGizmoMode, inactiveGizmoMode);
        if (changeGizmoModeClicked) showGizmoModeMenu = !showGizmoModeMenu;
        ...
        bool toggleSnapClicked =
            renderToolbarButton("toggle_snap", ICON_FA_MAGNET, "Toggle gizmo snap",
                                (showSnapToggleMenu || snapOn) ? m_selectedGradient : m_buttonGradient);
        if (toggleSnapClicked) showSnapToggleMenu = !showSnapToggleMenu;

        ImGui::SameLine();

        // -------- Grid enabled button --------
        bool rightClicked = false;
        components::RenderContext::GridParams& gridParams =
            Application::m_coordinator->getSingletonComponent<components::RenderContext>().gridParams;
        ...
        if (rightClicked) m_popupManager.openPopup("Grid settings", ImVec2(300, 180));
