    ...
    void MaterialProperty::createMaterialPopup(const ecs::Entity entity)
    {
        ...
        if (!materialRef) {
            ...
            materialRef           = assets::AssetCatalog::getInstance().createAsset<assets::Material>(
                assets::AssetLocation("_internal::TempMaterial@_internal"), std::move(material));
        }
        ...
        // --- Right Side: Material Preview ---
        {
            ...
            auto &materialComp =
                Application::m_coordinator->getComponent<components::MaterialComponent>(out.entityCopy);
        ...
        if (ImNexo::Button("OK", ImVec2(buttonWidth, 0))) {
            ...
            if (materialRef.isValid()) {
                ...
                const auto newMaterialRef = assets::AssetCatalog::getInstance().createAsset<assets::Material>(
                    finalLocation, std::make_unique<components::Material>(*materialRef.lock()->getData()));

                auto &materialComponent =
                    Application::m_coordinator->getComponent<components::MaterialComponent>(entity);
        ...
        ImGui::SameLine();
        if (ImNexo::Button("Cancel", ImVec2(buttonWidth, 0))) cleanupPopup(materialRef, out);
    }
    ...
    void MaterialProperty::show(const ecs::Entity entity)
    {
        ...
        if (ImNexo::Header("##MaterialNode", "Material Component")) {
            const ImTextureID textureID =
                ThumbnailCache::getInstance().getMaterialThumbnail(materialComponent.material);
