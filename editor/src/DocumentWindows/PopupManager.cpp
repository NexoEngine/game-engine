...
namespace nexo::editor {
    ...
    {
        const PopupProps props{.open = true, .callback = nullptr, .size = popupSize};
    ...
    {
        const PopupProps props{.open = true, .callback = std::move(callback), .size = popupSize};
        m_popups[popupName] = props;
    }
    ...
    {
        if (!m_popups.contains(popupName)) return false;
    ...
    {
        if (!m_popups.contains(popupModalName)) return false;
        ...
        if (!ImGui::BeginPopupModal(popupModalName.c_str(), nullptr, flags)) return false;
