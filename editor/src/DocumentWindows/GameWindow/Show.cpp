...
namespace nexo::editor {
    ...
    void GameWindow::show()
    {
        ...
        beginRender(std::format("###GameWindow{}", std::to_string(m_sceneId)));
    ...
    void GameWindow::renderToolbar()
    {
        ...
        // Stop button
        static const std::vector<ImNexo::GradientStop> standardGradient = {{0.0f, IM_COL32(50, 50, 70, 230)},
                                                                           {1.0f, IM_COL32(30, 30, 45, 230)}};

        if (ImNexo::IconGradientButton("stop_game", ICON_FA_STOP, ImVec2(buttonWidth, buttonHeight),
                                       standardGradient)) {
            m_opened = false;
        }
        ...
        if (m_isPaused) {
            // Resume button
            static const std::vector<ImNexo::GradientStop> selectedGradient = {{0.0f, IM_COL32(70, 70, 120, 230)},
                                                                               {1.0f, IM_COL32(50, 50, 100, 230)}};

            if (ImNexo::IconGradientButton("resume_game", ICON_FA_PLAY, ImVec2(buttonWidth, buttonHeight),
                                           selectedGradient)) {
                m_isPaused = false;
            }
            if (ImGui::IsItemHovered()) ImGui::SetTooltip("Resume game");
        } else {
            // Pause button
            if (ImNexo::IconGradientButton("pause_game", ICON_FA_PAUSE, ImVec2(buttonWidth, buttonHeight),
                                           standardGradient)) {
                m_isPaused = true;
            }
