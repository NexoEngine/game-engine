...
namespace nexo::editor {
    ...
    void ConsoleWindow::showVerbositySettingsPopup()
    {
        ...
        const struct {
            ...
            const char *name;
        } levels[] = {{loguru::Verbosity_FATAL, "FATAL"},
        ...
        if (ImNexo::Button("Open log folder")) utils::openFolder(Path::resolvePathRelativeToExe("../logs").string());
    ...
    void ConsoleWindow::calcLogPadding()
    {
        ...
        for (const auto &selectedLevel : m_selectedVerbosityLevels) {
            ...
            if (textSize.x > m_logPadding) m_logPadding = textSize.x;
        }
    ...
    void ConsoleWindow::show()
    {
        ...
        if (m_logPadding == 0.0f) calcLogPadding();

        auto id = 0;
        for (const auto &[verbosity, message, prefix] : m_logs) {
            if (!m_selectedVerbosityLevels.contains(verbosity)) continue;
        ...
        ImGui::SameLine();
        if (ImNexo::Button("...")) ImGui::OpenPopup("VerbositySettings");

        if (ImGui::BeginPopup("VerbositySettings")) showVerbositySettingsPopup();
