...
namespace nexo::editor {
    ...
    class ConsoleWindow final : public ADocumentWindow {
       public:
         ...
         *
         * Removes the registered loguru callback identified by LOGURU_CALLBACK_NAME to prevent further logging after
         ...
         * opened for the first time, it performs an initial docking setup. The function also adjusts log padding for
         * proper alignment and automatically scrolls to the bottom if new messages have been added.
         *
         * An input field is provided for entering commands, which are executed upon pressing Enter, with the input
         * buffer cleared afterward. Additionally, a popup for adjusting verbosity settings is available, accessible via
         * a button.
         */
        void show() override;
       ...
       private:
        ...
        std::set<loguru::Verbosity> m_selectedVerbosityLevels = {
            loguru::Verbosity_FATAL, loguru::Verbosity_ERROR, loguru::Verbosity_WARNING,
            loguru::Verbosity_INFO,  loguru::Verbosity_1,
        };
        ...
        void addLog(std::format_string<Args...> fmt, Args &&...args)
        {
            try {
                ...
                auto result = std::format_to_n(std::begin(errorBuffer), std::size(errorBuffer) - 1,
                                               "Error formatting log message: {}", e.what());
