...
namespace nexo::editor {
    void ConsoleWindow::loguruCallback([[maybe_unused]] void *userData, const loguru::Message &message)
    {
    ...
    ConsoleWindow::ConsoleWindow(const std::string &windowName, WindowRegistry &registry)
        : ADocumentWindow(windowName, registry)
    {
        loguru::add_callback(LOGURU_CALLBACK_NAME, &ConsoleWindow::loguruCallback, this, loguru::Verbosity_MAX);
        ...
        auto engineLogCallback = [](const LogLevel level, const std::source_location &loc, const std::string &message) {
            ...
            if (loguruLevel > loguru::current_verbosity_cutoff()) return;
        ...
        try {
            ...
            if (logFilePath.empty()) {
                THROW_EXCEPTION(LogFilePathEmptyException);
            }

            const auto resolvedPath = Path::resolvePathRelativeToExe(logFilePath);
            if (resolvedPath.empty()) {
                THROW_EXCEPTION(LogResolvedPathEmptyException);
            }
    ...
    {
        // All the setup is made in the constructor because the rest of the editor needs the log setup before setting up
        // the windows
    }
} // namespace nexo::editor
