...
namespace nexo::editor {
    ...
    class InspectorWindow final : public ADocumentWindow {
       public:
        ...
        /** @brief Sets the visibility flag for a specific sub-inspector type.
         *
         * This function updates the visibility flag for the sub-inspector designated by the template
         * parameter T. The visibility state is stored in a map using the type index of T as the key.
         *
         ...
         *
         * This function checks if a visibility flag has been set for the sub-inspector designated by the template
         * parameter T. If no flag is found, it returns false.
         *
         * @tparam T The type of the sub-inspector.
         * @return true if the sub-inspector is marked as visible; otherwise, false.
         */
        template<typename T>
        [[nodiscard]] bool getSubInspectorVisibility() const
        {
         ...
         * If an entry for WindowType exists, it returns the associated pointer to a Data type; otherwise, it returns
         * nullptr
         *
        ...
        std::optional<Data> getSubInspectorData() const
        {
            ...
            if (it != m_subInspectorData.end()) {
                ...
                LOG(NEXO_ERROR, "Failed to cast sub-inspector data for type {}", typeid(WindowType).name());
                return std::nullopt;
            }
            return std::nullopt;
        }
       ...
       private:
         ...
         *
         * Retrieves the scene corresponding to the provided SceneId and renders UI controls that allow toggling the
         * scene's render and active statuses. The UI is configured with two columns where the "Hide" checkbox inverts
         * the scene's rendering state and the "Pause" checkbox inverts its active state. An icon prefix is removed from
         * the scene's UI handle before display.
         *
        ...
        /**
         * @brief Registers a property handler for a specific component type.
         *
         * This templated method allows registering a property handler for a given component type by
         * specifying the component and property types. The property type must derive from AEntityProperty.
         * The method retrieves the component type identifier from the coordinator and associates it with
         * a new instance of the specified property type, constructed with the current InspectorWindow instance.
         *
         * @tparam Component The component type for which the property handler is being registered.
         * @tparam Property The property handler type that must derive from AEntityProperty.
         */
         ...
         * @brief Registers a property handler for a specific component type.
         *
         * This method allows registering a property handler for a given component type by directly providing
         * the component type identifier and a shared pointer to an IEntityProperty instance. If the provided
         * property pointer is null, an error is logged and the registration is aborted.
         *
         * @param type The component type identifier for which the property handler is being registered.
         * @param property A shared pointer to the IEntityProperty instance that handles the properties of the specified
         * component type.
         */
