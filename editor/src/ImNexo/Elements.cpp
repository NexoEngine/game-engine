...
namespace ImNexo {
     ...
     */
    void CenteredIcon(const std::string& icon, const ImVec2& p_min, const ImVec2& p_max, const ImU32 color,
                      const float scale, const float verticalPosition, const float horizontalPosition, ImFont* font)
    {
        ...
        // Calculate position
        const auto iconPos = ImVec2(p_min.x + (p_max.x - p_min.x - iconSize.x) * horizontalPosition,
                                    p_min.y + (p_max.y - p_min.y) * verticalPosition - iconSize.y * 0.5f);

        // Draw the icon
        drawList->AddText(font ? font : ImGui::GetFont(), ImGui::GetFontSize() * scale, iconPos, color, icon.c_str());
     ...
     */
    void WrappedCenteredText(const std::string& text, const ImVec2& p_min, const ImVec2& p_max, const ImU32 color,
                             const float verticalPosition)
    {
        ...
        if (textSize.x > wrapWidth) {
            ...
            if (splitPos != std::string::npos) {
                ...
                const auto line1Pos =
                    ImVec2(p_min.x + (p_max.x - p_min.x - ImGui::CalcTextSize(line1.c_str()).x) * 0.5f,
                           textY - textHeight * 0.5f);

                const auto line2Pos =
                    ImVec2(p_min.x + (p_max.x - p_min.x - ImGui::CalcTextSize(line2.c_str()).x) * 0.5f,
                           textY + textHeight * 0.5f);
                ...
                const auto textPos =
                    ImVec2(p_min.x + (p_max.x - p_min.x - textSize.x) * 0.5f, textY - textHeight * 0.5f);
                drawList->AddText(textPos, color, text.c_str());
            }
        } else {
            // No wrapping needed, draw centered
            const auto textPos = ImVec2(p_min.x + (p_max.x - p_min.x - textSize.x) * 0.5f, textY - textHeight * 0.5f);
            drawList->AddText(textPos, color, text.c_str());
        }
    }
                ...
                const ImU32 bgActive, const ImU32 txtColor)
    {
        ...
        colorGuard.push(ImGuiCol_ButtonHovered, bgHovered)
                   ...
                   const ImU32 textColor)
    {
        ...
        colorGuard.push(ImGuiCol_FrameBgHovered, bgHovered)
    ...
    static void sanitizeGradientStops(std::vector<GradientStop>& stops)
    {
        ...
        std::ranges::sort(stops, [](const GradientStop& a, const GradientStop& b) { return a.pos < b.pos; });
    ...
    bool Header(const std::string& label, const std::string_view headerText)
    {
        ...
        bool open = ImGui::TreeNodeEx(label.c_str(), ImGuiTreeNodeFlags_DefaultOpen | ImGuiTreeNodeFlags_Framed |
        ...
        ImGui::GetWindowDrawList()->AddText(ImGui::GetFont(), ImGui::GetFontSize(), textPos,
                                            ImGui::GetColorU32(ImGuiCol_Text), headerText.data());
    ...
    void Arrow(const ImVec2& center, const bool isExpanded, const ImU32 color, const float size)
    {
        ...
        if (isExpanded) {
            ...
            draw_list->AddTriangleFilled(ImVec2(center.x - size, center.y - size),
                                         ImVec2(center.x + size, center.y - size), ImVec2(center.x, center.y + size),
                                         color);
        } else {
            // Rightward arrow (collapsed)
            draw_list->AddTriangleFilled(ImVec2(center.x - size, center.y - size),
                                         ImVec2(center.x - size, center.y + size), ImVec2(center.x + size, center.y),
                                         color);
        }
    }
