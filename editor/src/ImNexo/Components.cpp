...
//  Author:      Mehdy MORVAN
//  Date:        17/02/2025
//  Description: Source file for the utility ImGui functions
//
///////////////////////////////////////////////////////////////////////////////
...
namespace ImNexo {
                               ...
                               const ImVec2 &itemSize)
    {
        ...
        const std::string invisibleButtonLabel = "##" + uniqueId;

        if (ImGui::InvisibleButton(invisibleButtonLabel.c_str(), itemSize)) return true;

        // Draw the background
        auto [p0, p1] = utils::getItemRect();
        ImGui::GetWindowDrawList()->AddRectFilled(
            p0, p1, ImGui::GetColorU32(ImGui::IsItemHovered() ? ImGuiCol_ButtonHovered : ImGuiCol_Button),
            ImGui::GetStyle().FrameRounding);

        // Draw the icon at 25% from top
        CenteredIcon(icon, p0, p1, ImGui::GetColorU32(ImGuiCol_Text), 1.5f, 0.25f, 0.5f);

        // Draw the label with wrapping if needed
        WrappedCenteredText(label, p0, p1, ImGui::GetColorU32(ImGuiCol_Text),
                            0.6f // Position at 60% from top
        );

        // Use drawButtonBorder instead of direct drawing
        ButtonBorder(0, // Use default color
                     ImGui::GetColorU32(ImGuiCol_ButtonHovered), ImGui::GetColorU32(ImGuiCol_ButtonActive),
                     ImGui::GetStyle().FrameRounding);

        return false;
    }

    void ColorButton(const std::string &label, const ImVec2 size, const ImVec4 color, bool *clicked,
                     ImGuiColorEditFlags flags)
    {
        ...
        const auto calculatedSize       = ImVec2(size.x == 0 ? defaultSize : size.x - borderThickness * 2,
        ...
        ButtonBorder(ImGui::GetColorU32(ImGuiCol_Button), ImGui::GetColorU32(ImGuiCol_ButtonHovered),
                     ImGui::GetColorU32(ImGuiCol_ButtonActive), borderThickness);
    }
                       ...
                       std::filesystem::path &outPath)
    {
        ...
        if (ImageButton(textureButton.c_str(), textureId, previewSize)) {
            const char *filePath = tinyfd_openFileDialog("Open Texture", "", 0, nullptr, nullptr, 0);
        ...
        ButtonBorder(IM_COL32(255, 255, 255, 0), IM_COL32(255, 255, 255, 255), IM_COL32(255, 255, 255, 0), 0.0f, 0,
                     2.0f);
        ImGui::PopID();
        ImGui::SameLine();
    ...
    bool IconGradientButton(const std::string &uniqueId, const std::string &icon, const ImVec2 &size,
                            const std::vector<GradientStop> &gradientStops, const float gradientAngle,
                            const ImU32 borderColor, const ImU32 borderColorHovered, const ImU32 borderColorActive,
                            const ImU32 iconColor)
    {
        ...
        ButtonBorder(borderColor, borderColorHovered, borderColorActive,
                     3.0f, // rounding
                     0,    // no flags
                     1.5f  // thickness
        );
    ...
    bool RowDragFloat(const Channels &channels)
    {
        ...
        for (unsigned int i = 0; i < channels.count; ++i) {
            ...
            if (!channels.badges[i].label.empty()) {
                ...
                Button(badge.label, badge.size, badge.bg, badge.bgHovered, badge.bgActive, badge.txtColor);
                ImGui::SameLine(0, 2);
            }
            ...
            const bool changed =
                DragFloat(slider.label, slider.value, slider.speed, slider.min, slider.max, slider.format, slider.bg,
                          slider.bgHovered, slider.bgActive, slider.textColor);
    ...
    bool RowDragFloat1(const char *uniqueLabel, const std::string &badgeLabel, float *value, float minValue,
                       float maxValue, float speed)
    {
        ImGui::TableNextRow();
        ...
        Channels channels;
        channels.count = 1;
        channels.badges.push_back({badgeId,
                                   {0, 0},
                                   IM_COL32(80, 0, 0, 255),
                                   IM_COL32(80, 0, 0, 255),
                                   IM_COL32(80, 0, 0, 255),
        ...
        channels.sliders.emplace_back(labelId, value, speed, minValue, maxValue, 0, 0, 0, 0, "%.2f");
    ...
    bool RowDragFloat2(const char *uniqueLabel, const std::string &badLabelX, const std::string &badLabelY,
                       float *values, const float minValue, const float maxValue, float speed,
                       std::vector<ImU32> badgeColor, std::vector<ImU32> textBadgeColor, const bool disabled)
    {
        ImGui::TableNextRow();
        ...
        const ImU32 textColor =
            disabled ? ImGui::GetColorU32(ImGuiCol_TextDisabled) : ImGui::GetColorU32(ImGuiCol_Text);
        const ImU32 bgColor        = ImGui::GetColorU32(ImGuiCol_FrameBg);
        const ImU32 bgHoveredColor = ImGui::GetColorU32(ImGuiCol_FrameBgHovered);
        const ImU32 bgActiveColor  = ImGui::GetColorU32(ImGuiCol_FrameBgActive);

        // Slider controls
        channels.sliders = {{"##X" + baseId, &values[0], speed, minValue, maxValue, bgColor, bgHoveredColor,
                             bgActiveColor, textColor, "%.2f"},
                            {"##Y" + baseId, &values[1], speed, minValue, maxValue, bgColor, bgHoveredColor,
                             bgActiveColor, textColor, "%.2f"}};
    ...
    bool RowDragFloat3(const char *uniqueLabel, const std::string &badLabelX, const std::string &badLabelY,
                       const std::string &badLabelZ, float *values, const float minValue, const float maxValue,
                       float speed, std::vector<ImU32> badgeColors, std::vector<ImU32> textBadgeColors)
    {
        ImGui::TableNextRow();

        // Set up the label for the row
        ChannelLabel chanLabel;
        chanLabel.label = uniqueLabel;
        ...
        // Badge labels
        channels.badges = {{badLabelX + "##" + baseId,
                            {badgeSize, badgeSize},
                            badgeColors[0],
                            badgeColors[0],
                            badgeColors[0],
        ...
        // Slider controls
        channels.sliders = {{"##X" + baseId, &values[0], speed, minValue, maxValue, 0, 0, 0, textColor, "%.2f"},
                            {"##Y" + baseId, &values[1], speed, minValue, maxValue, 0, 0, 0, textColor, "%.2f"},
                            {"##Z" + baseId, &values[2], speed, minValue, maxValue, 0, 0, 0, textColor, "%.2f"}};

        if (!chanLabel.label.empty()) RowLabel(chanLabel);
    ...
    bool ToggleButtonWithSeparator(const std::string &label, bool *toggled)
    {
        ...
        ImGui::GetWindowDrawList()->AddLine(separatorPos, ImVec2(separatorPos.x, separatorPos.y + separatorHeight),
        ...
        CustomSeparatorText(label, 10.0f, 0.1f, 0.5f, IM_COL32(255, 255, 255, 255), IM_COL32(255, 255, 255, 255));
