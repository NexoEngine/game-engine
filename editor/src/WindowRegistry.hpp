...
namespace nexo::editor {
    ...
    class WindowRegistry {
       public:
        /**
         * @brief Initializes all registered document windows.
         *
         * This function iterates through all registered document windows and calls their setup method
         * to initialize them. It ensures that each window is properly set up before use.
         */
        [[nodiscard]] bool hasWindow(const std::string &windowName) const
        {
            for (const auto &[type, windows] : m_windows) {
                if (std::ranges::any_of(
                        windows, [&windowName](const auto &window) { return window->getWindowName() == windowName; })) {
                    return true;
                }
            }
            return false;
        ...
        void unregisterWindow(const std::string &windowName)
        {
            ...
            auto found          = std::ranges::find_if(
                windowsOfType, [&windowName](const auto &w) { return w->getWindowName() == windowName; });
        ...
        template<typename T>
            requires std::derived_from<T, IDocumentWindow>
        std::ranges::transform_view<std::ranges::ref_view<const std::vector<std::shared_ptr<IDocumentWindow>>>,
        ...
        template<typename T>
            requires std::derived_from<T, IDocumentWindow>
        std::ranges::transform_view<std::ranges::ref_view<std::vector<std::shared_ptr<IDocumentWindow>>>,
         ...
         *
         * Registers the specified docking identifier for the window identified by its name by delegating to the docking
         * registry.
         *
         ...
         */
        [[nodiscard]] std::optional<ImGuiID> getDockId(const std::string &name) const;

        /**
         * @brief Retrieves the currently focused window.
         *
         * This function iterates through all registered windows and returns a shared pointer to the window
         * that is currently focused. If no window is focused, it returns a nullptr.
         *
         * @return std::shared_ptr<IDocumentWindow> A shared pointer to the focused window, or nullptr if none are
         * focused.
         */
        [[nodiscard]] std::shared_ptr<IDocumentWindow> getFocusedWindow() const;
         ...
         *
         * Iterates through the registered windows and invokes the show() method on each window that is currently
         * opened.
         */
        void render() const;
